...
static ios_status enumerate_doc_files(
    void *context,
...
{
    struct ios_doc_viewer_application *application = context;
    return ios_file_explorer_client_file_view(
        &application->client,
        IOS_SHELL_TYPE_VIEW,
        directory_handle,
        application->doc_type_capability,
        entries,
        capacity,
        entry_count
    );
...
static ios_status resolve_doc_icon(
    void *context,
...
)
{
    ...
    if (application == NULL || doc_type_capability == IOS_INVALID_TYPE_ICON_CAPABILITY
    ...
    application->doc_type_capability = doc_type_capability;
    ...
    application->window.title = "DOC Files App";
    return IOS_OK;
}
